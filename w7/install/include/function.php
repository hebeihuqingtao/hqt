...
function install_sql()
{
    ...
    $location=substr($db_address,0,strpos($db_address,':'));
    $pore=substr($db_address,strpos($db_address,':')+1);    //端口号
	$db_user      = url_get('db_user');
	$db_pwd       = url_get('db_pwd');
	...
	//插入管理员数据
	$adminSql = 'insert into `'.$db_pre.'admin` (`u_name`,`u_pwd`,`create_time`) values ("'.$admin_user.'","'.md5($admin_pwd).'","'.date('Y-m-d H:i:s').'")';
    ...
    $updateData ="<?php
        return [
            ...
            'dsn' => 'mysql:host=$location;port=$pore;dbname=$db_name',
            'username' => '$db_user',
            'password' => '$db_pwd',
            'charset' => 'utf8',
            'tablePrefix' => 'wx_',   //加入前缀名称fc_
